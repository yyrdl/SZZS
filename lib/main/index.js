...
var local_ip=require("../tool").localIPV4Adress;
var targetModules={
    "http":{
        "spy":Spy.httpSpy,
        "done":false
...
_module._load=function(){
    ...
    if(targetModules[module_name]&&!targetModules[module_name].done)
    {
        ...
        targetModules[module_name].done=true;//防止重复包装,avoid hook repeatedly
    }
...
module.exports={
    ...
    "promiseProxy":function(ori_method,config){
        ...
        return function(){
            ...
            if(__self__.config_data.project_name!=="undefined"){
                record.project_name=__self__.config_data.project_name;
            }
            if(config.input) {
